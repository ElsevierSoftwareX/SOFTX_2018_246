% Tests for complementary complete ellitic integral of second kind

fprintf('Check values ************** \n')
m = [80/81, 5e-4, 3e6];
fex = [1.56593690932295903, ...
      1.00117187880359293,...
       1732.05321608851135];
for n = 1:length(m)
    f = mEllipticCE(m(n));
    rer = abs(f/fex(n)-1);
    fprintf('melCE(%g) \t = %.16g \t rerr = %g\n',m(n),f,rer)    
end
k = (sqrt(3)-1)/(2*sqrt(2));
f = EllipticCE(k);
fex = pi*sqrt(3)/4/elCK(k) + sqrt(2/3)*k*elCK(k);
rer = abs(f/fex-1);
fprintf('elCE(%g) \t = %.16g \t rerr = %g\n',k,f,rer)
fprintf('elCE(%g) \t = %.16g \t rerr = %g\n',0,melCE(0),melCE(0)-1)
fprintf('elCE(%g) \t = %.16g \t rerr = %g\n',1,melCE(1),melCE(1)/(pi/2)-1)
fprintf('elCE(%g) \t = %.16g \t rerr = %g\n',inf,melCE(inf),isinf(melCE(inf))-1)

   cfun = {...
       'melCE(m)/melE(1 - m) - 1',...
       'elCE(m)/elE(sqrt(1 - m^2)) - 1', ...      
       'melCE(m^2)/ elCE(m) - 1', ...    
       'elCE(m) - elCE(-m)',... 
       };      
